...
Card = function (stack, targetElement) {
    ...
    mc.on('panmove', function (e) {
        ...
        xconf = Card.throwOutConfidence(x, targetElementWidth);
        yconf = Card.throwOutConfidence(y, targetElementHeight);
        if (xconf > yconf) {
            defconf = xconf;
            dir = x < 0 ? Card.DIRECTION_LEFT : Card.DIRECTION_RIGHT
        } else {
            defconf = yconf;
            dir = y > 0 ? Card.DIRECTION_DOWN : Card.DIRECTION_UP
        }
        eventEmitter.trigger('dragmove', {
            target: targetElement,
            throwOutConfidence: defconf,
            throwDirection: dir,
        });
    });
    ...
    mc.on('panend', function(e) {
        ...
        if (config.isThrowOut(x, targetElementWidth)) {
            ...
            if (config.isThrowOut(y, targetElementHeight)) {
                card.throwOut(x, y, 1);
            } else {
                card.throwIn(x, y);
            }
    ...
    card.throwIn = function (fromX, fromY) {
        throwWhere(Card.THROW_IN, fromX, fromY, 0);
    };
     ...
     * @param {Number} fromY
     */
    card.throwOut = function (fromX, fromY, dir) {
        throwWhere(Card.THROW_OUT, fromX, fromY, dir);
    };
     ...
     * @param {Number} fromY
     */
    throwWhere = function (where, fromX, fromY, dir) {
        ...
        if (dir == 0) {
            ...
            lastThrow.direction = lastThrow.fromY > 0 ? Card.DIRECTION_DOWN : Card.DIRECTION_UP;
        }
        ...
        if (where == Card.THROW_IN) {
            ...
            if (dir == 0) {
                ...
                springThrowOut.setCurrentValue(0).setAtRest().setVelocity(1).setEndValue(0);
            }
            ...
            if (lastThrow.direction == Card.DIRECTION_RIGHT) {
                eventEmitter.trigger('throwoutright', {
                    target: targetElement,
                    throwDirection: lastThrow.direction
                });
            }
            if (lastThrow.direction == Card.DIRECTION_UP) {
                eventEmitter.trigger('throwoutup', {
                    target: targetElement,
                    throwDirection: lastThrow.direction
                });
            }
            if (lastThrow.direction == Card.DIRECTION_DOWN) {
